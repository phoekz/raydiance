...
pub fn build() -> Result<()> {
    ...
    // Generate tone mapping plot for blog.
    {
        ...
        // Hack: we are going to embed the svg directly into the Markdown file
        // because we want to reuse our font-family which are defined in the
        // main CSS. <object> and <img> embeds don't have this capability. This
        // is why we need to perform two hacks to make the embed work:
        // - Collapse everything into a single line so the diffs are more
        //   readable.
        // - Change the size of the <svg> object, because plotters doesn't seem
        //   to be able to change it.
        File::create("work/raw.svg")?.write_all(
            output_svg
                .replace('\n', "")
                .replace(
